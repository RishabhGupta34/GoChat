...
func (c *Client) JoinChannel(ServerVars *Variables, ch string, k string, t time.Time) {
	...
	ServerVars.ChannelList[ch].UserList[c.Username] = c.Conn // Adding the client to the members list of the channel
	ServerVars.ActivityLogsChannel <- fmt.Sprintf(ServerVars.LogActivityFormat, "Join Channel", "", ch, c.Username, t.Format("2006-01-02 15:04:05"))
}
...
func (c *Client) LeaveChannel(ServerVars *Variables, ch string, t time.Time) {
	...
	ServerVars.ActivityLogsChannel <- fmt.Sprintf(ServerVars.LogActivityFormat, "Leave Channel", "", ch, c.Username, t.Format("2006-01-02 15:04:05")) // Log Activity
}
...
func (c *Client) Info(ServerVars *Variables, ch string) {
	...
	ServerVars.ActivityLogsChannel <- fmt.Sprintf(ServerVars.LogActivityFormat, "Print Channel Info", "", ch, c.Username, t) // Log Activity
	txt := "Channel name:- %s\nAccess:- %s\nKey:- %s\nCreated by:- %s\nCreated At:- %s\nMembers:-\n"
